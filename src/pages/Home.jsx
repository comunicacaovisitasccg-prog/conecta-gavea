...
const SELOS = [
  {
    linhas: ['Experiência', 'autêntica'],
    icone: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#6f8a3f" strokeWidth="1.6">
  ...
  {
    linhas: ['Segurança em', 'primeiro lugar'],
    icone: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#6f8a3f" strokeWidth="1.6">
  ...
  {
    linhas: ['Conhecimento', 'que conecta'],
    icone: (
      <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#6f8a3f" strokeWidth="1.6">
  ...
  useEffect(() => {
    async function carregar() {
      try {
        ...
        if (configSnap.exists() && configSnap.data().visitantesParticipantes) {
          setVisitantes(configSnap.data().visitantesParticipantes)
        }
      } finally {
        setCarregando(false)
      }
      ...
      <div className="max-w-2xl mx-auto bg-white min-h-screen">
        ...
        <div className="relative overflow-hidden bg-mint">
          ...
          <div
            ...
            style={{
              background:
                'linear-gradient(to right, #f6f9f0 22%, rgba(246,249,240,0.85) 42%, rgba(246,249,240,0.35) 65%, rgba(246,249,240,0) 95%)',
            }}
          />
          <div className="relative z-10 px-6 py-7">
            <h1 className="font-poppins font-extrabold text-navy leading-[1.05] text-[32px] mb-3">
              Conecta
              <br />
              <span className="text-green">Gávea</span>
            </h1>
            <p className="text-[13px] text-navy/70 mb-5">
              Visita guiada à <strong className="text-navy">Estação Gávea</strong> · Linha 4
            </p>
            ...
            <div className="flex items-start justify-between gap-2 max-w-[300px]">
              {SELOS.map((selo) => (
                <div key={selo.linhas[0]} className="flex-1 flex flex-col items-center text-center gap-1.5">
                  <div className="w-12 h-12 rounded-full border border-green flex items-center justify-center bg-white/70">
                    {selo.icone}
                  </div>
                  <p className="text-[10.5px] font-semibold text-navy leading-tight">
                    {selo.linhas[0]}
                    <br />
                    {selo.linhas[1]}
                  </p>
        ...
        <div className="px-6 pt-6">
          ...
          {carregando ? (
            <p className="text-sm text-navy/35">Carregando visitas...</p>
          ) : visitas.length === 0 ? (
            <p className="text-sm text-navy/35">Nenhuma visita programada no momento.</p>
          ) : (
              ...
              {visitas.map((visita) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center justify-between px-3.5 py-3 rounded-[10px] border transition ${
                      lotada
                        ? 'border-navy/[0.08] opacity-50 cursor-not-allowed'
                        : 'border-navy/10 hover:border-navy/25'
                    }`}
                  >
                    <span className="flex items-center gap-2.5">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#6f8a3f" strokeWidth="1.6">
                    ...
                    <span className="flex items-center gap-2">
                      <span className={`text-xs font-semibold ${lotada ? 'text-navy/35' : 'text-green'}`}>
                        {lotada ? 'Esgotado' : `${visita.restantes} vagas`}
                      </span>
                      <svg width="14" height="14" viewBox="0 0 24 24" fill="none">
                        <path
                          d="M9 6l6 6-6 6"
                          stroke="rgba(18,33,58,0.35)"
                          strokeWidth="2"
                          strokeLinecap="round"
          ...
          <div className="flex items-center gap-3.5 bg-mint rounded-xl px-3.5 py-3 mb-7" style={{ border: '1px solid rgba(139,197,63,0.3)' }}>
            ...
            <div>
              <p className="text-xs font-semibold text-navy mb-0.5">Vem conhecer a obra!</p>
              <p className="text-xs text-navy/55 leading-snug">
                Inscreva-se na próxima visita guiada à Estação Gávea.
              </p>
            </div>
          </div>

          <h2 className="text-[15px] font-bold text-navy mb-0.5">Veja como foi a última visita</h2>
          <p className="text-[12.5px] text-navy/50 mb-2.5">
            Curiosos com a obra? Garanta seu lugar na próxima.
          </p>
          <div className="relative mb-4">
            <Carrossel fotos={fotos} />
            {fotos.length > 0 && (
              <span className="absolute bottom-2.5 left-2.5 bg-greenfill text-navy text-[11px] font-bold px-2.5 py-1 rounded-full">
                +{visitantes} visitantes já participaram
              </span>
            )}
          </div>
        </div>
